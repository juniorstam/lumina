  ...
  if (step === 'scanning') {
    return (
      <div className="w-full max-w-sm text-center">
        <div className="bg-[#111113] border border-zinc-800 rounded-2xl p-10">
          ...
          <h2 className="text-lg font-semibold text-zinc-100 mb-2">Escaneando seus e-mails...</h2>
          <p className="text-sm text-zinc-500 mb-6">A Lumina está vasculhando seu histórico em busca de cursos e certificados.</p>
          <div className="space-y-2 text-left">
            {[
              { label: 'Conectando ao Gmail', done: true },
              { label: 'Buscando compras de cursos', done: true },
              { label: 'Identificando certificados', done: false },
              { label: 'Organizando sua biblioteca', done: false },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-3 text-sm">
                ...
                <span className={item.done ? 'text-zinc-400' : 'text-zinc-300'}>{item.label}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="w-full max-w-sm">
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-xs text-zinc-400 mb-4">
          ...
          Encontre seus cursos esquecidos em minutos
        </div>
      </div>
      ...
      <div className="bg-[#111113] border border-zinc-800 rounded-2xl p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-semibold text-zinc-100 mb-1.5">Crie sua conta</h1>
          <p className="text-sm text-zinc-500">Comece a organizar seu portfólio intelectual</p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-xs text-zinc-600">Ou cadastre-se com e-mail</span>
          <div className="flex-1 h-px bg-zinc-800" />
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1.5">Endereço de e-mail</label>
            <input
              type="email"
          ...
          <div className="bg-zinc-900/60 rounded-lg p-3.5 space-y-2">
            {[
              'Escaneie todas as suas contas de e-mail em busca de cursos',
              'Organize tudo automaticamente — sem entrada manual',
              'A IA encontra cursos esquecidos e sugere os próximos passos',
            ].map((item) => (
              <div key={item} className="flex items-start gap-2">
                <svg className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 16 16">
                  <path d="M3 8l3 3 7-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-lg border border-blue-500/50 transition-all disabled:opacity-70"
          >
            {loading ? (
              <>
